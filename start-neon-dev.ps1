...
function Wait-ForBackend {
    ...
    Write-Error "Backend did not become healthy. Check .logs\backend.log and .logs\backend.err.log"
    exit 1
}

$envFile = Join-Path $Root "backend\.env"
if (-not (Test-Path $envFile)) {
    Write-Error "Missing backend\.env. Add Neon DATABASE_URL, DATABASE_USERNAME, and DATABASE_PASSWORD first."
    exit 1
}
...
Write-Host ""
Write-Host "Memoris OS is running with Neon PostgreSQL."
